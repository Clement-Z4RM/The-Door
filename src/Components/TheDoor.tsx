...
export default function TheDoor() {
    ...
    const updateTemperature = useCallback((callback?: () => void) => {
        ...
        void fetch(
            ...
            .then((data: MeteoConceptResponse) => {
                ...
                if (callback)
                    callback()
                setAnswers(Array(4).fill(0).map((_, index) => index === townIndex ? town : towns[Math.floor(Math.random() * towns.length)]))
            })
    }, [towns])
    ...
    return (
        <div className={"h-screen w-screen flex flex-col items-center justify-center"}>
            ...
            <p className={'pt-3 font-black text-xl'}>
                Score: {score}
            </p>
            <img
            ...
            <p className={'font-bold'} hidden={answers.length === 0}>
                De quelle ville provient la température affichée ? 🤔
            </p>
            <TownButtons answers={answers} temperature={temperature} setPopup={setPopup} setScore={setScore}/>
            <Popup
                open={popup != PopupState.Closed}
                close={() => {
                    if (popup == PopupState.Success)
                        updateTemperature()
                    else {
                        setDoor((door) => door === DOOR.CLOSED ? DOOR.OPEN : DOOR.CLOSED)
                        setAnswers([])
